...
/**
 * What an API key is allowed to reach.
 *
 * An allowlist rather than a denylist, so a route added later is refused until
 * somebody decides otherwise — the failure mode of a denylist is a new route
 * quietly becoming reachable. Content and media only: a key cannot change the
 * password, sign anyone out, mint or revoke a key, touch the Instagram and
 * Anthropic credentials, or export the site.
 */
function apiKeyAllows(method, adminRoute) {
  if (adminRoute === '/site') return method === 'GET' || method === 'PUT';
  if (adminRoute === '/uploads') return method === 'GET' || method === 'POST';
  if (adminRoute.startsWith('/uploads/')) return method === 'DELETE';
  return false;
}

async function requireSession(req, res, adminRoute) {
  // A bearer key is the other way in, and it is checked first because a
  // request carrying one is not a browser request: no cookie is involved, so
  // there is no CSRF to defend against — a cross-site page cannot set this
  // header in the first place.
  const bearer = /^Bearer\s+(.+)$/i.exec(String(req.headers.authorization || ''));
  if (bearer) {
    const ip = clientIp(req);
    const blockedFor = auth.loginBlocked(ip);
    if (blockedFor) {
      sendJson(res, 429, {
        error: `Too many bad keys. Try again in ${Math.ceil(blockedFor / 60000)} minutes.`
      });
      return null;
    }
    if (!(await auth.checkApiKey(bearer[1].trim()))) {
      // The same counter the login form uses, so guessing a key is bounded too.
      auth.recordFailure(ip);
      sendJson(res, 401, { error: 'That API key is not valid' });
      return null;
    }
    if (!apiKeyAllows(req.method, adminRoute)) {
      sendJson(res, 403, {
        error: 'An API key may only read and write content and media. Sign in for anything else.'
      });
      return null;
    }
    auth.clearFailures(ip);
    return { token: null, session: { csrf: null }, viaApiKey: true };
  }
...
async function handleApi(req, res, url) {
  ...
  const ctx = await requireSession(req, res, adminRoute);
  ...
  // A key for editing content without a browser. Minting one replaces any
  // previous key, and the plaintext is returned exactly once — only its hash
  // is stored, so there is nothing to show again later.
  if (adminRoute === '/apikey' && req.method === 'POST') {
    const body = await readJson(req);
    const key = await auth.createApiKey(body.label);
    return sendJson(res, 200, {
      ok: true,
      key,
      ...auth.apiKeyStatus(await store.readSite())
    });
  }

  if (adminRoute === '/apikey' && req.method === 'DELETE') {
    await auth.revokeApiKey();
    return sendJson(res, 200, { ok: true, set: false });
  }
