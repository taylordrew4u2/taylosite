...
function status(site, env = process.env) {
  ...
  return {
    ...
    source: saved && saved.apiKey ? 'panel' : key ? 'environment' : null,
    model: MODEL
  };
}
...
async function saveKey({ store, apiKey: key }) {
  ...
  await store.update((site) => {
    site.auth = site.auth || {};
    site.auth.anthropic = { apiKey: value };
    return site;
  });
}
...
async function forgetKey(store) {
  await store.update((site) => {
    if (site.auth) delete site.auth.anthropic;
    return site;
  });
}
...
async function extract({ buffer, contentType, env = process.env, site = null, today = new Date().toISOString().slice(0, 10) }) {
  const key = apiKey(env, site);
  if (!key) throw Object.assign(new Error('Reading flyers needs an Anthropic API key. Paste one under Shows → Post a flyer, or set ANTHROPIC_API_KEY.'), { status: 400 });
  ...
  // Vercel cuts the function off at 60 s; answering with an error before then
  // beats a dropped connection. One retry, so a blip does not cost the read.
  ...
  try {
    ...
    if (err instanceof Anthropic.RateLimitError) {
      throw Object.assign(new Error('Anthropic is rate-limiting this key right now. Try again in a minute.'), { status: 429 });
    }
    if (err instanceof Anthropic.APIConnectionTimeoutError) {
      throw Object.assign(new Error('Anthropic took too long to answer. Try the flyer again, or a smaller image.'), { status: 504 });
    }
    if (err instanceof Anthropic.APIError) {
      throw Object.assign(new Error(`Could not read the flyer: ${err.message}`), { status: 502 });
    }
    throw Object.assign(new Error(`Could not reach Anthropic: ${err.message}`), { status: 502 });
  }
