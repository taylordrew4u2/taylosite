...
function createServerlessBackend(config) {
  ...
  // The Blob SDK is only required when images are actually touched, so a
  // deployment without Blob configured still serves and edits text content.
  let blob = null;
  function blobApi() {
    if (!blobToken) {
      throw new Error(
        'Image uploads need Vercel Blob. Add the Blob integration to this project so BLOB_READ_WRITE_TOKEN is set, then redeploy.'
      );
    }
  ...
  return {
    name: 'serverless',
    description: `Redis${blobToken ? ' + Vercel Blob' : ' (no Blob token — image uploads disabled)'}`,
    ensureDirs() {},
    ...
    async deleteImage(name) {
      const { del } = blobApi();
      const target = (await this.listImages()).find((image) => image.name === name);
