...
function createBackend(env = process.env) {
  ...
  if (env.VERCEL) {
    ...
    throw new Error(
      'Running on Vercel with nowhere to store content. Either add GITHUB_TOKEN so the site keeps its ' +
        'content in this repository (no other service needed), or connect a Redis store which sets ' +
        'KV_REST_API_URL and KV_REST_API_TOKEN. Then redeploy. See README → Deploying to Vercel.'
    );
  }
