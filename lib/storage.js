...
/**
 * Which storage credentials this build can see — presence only, never values.
 * Environment variables are baked in at build time, so this is the quickest way
 * to tell whether a deployment actually picked up a newly added integration.
 */
function credentialSummary(env = process.env) {
  ...
  return {
    ...
    github: Boolean(github.token && github.owner && github.repo)
  };
}
