...
function siteUrls(origin) {
  ...
  return ['/', '/about', '/shows', '/links', '/reels', '/photos', '/contact'].map((p) => (p === '/' ? `${base}/` : base + p));
}

/**
 * Remember how the last announcement went. Fire-and-forget is the right
 * behaviour for a save, but "it silently did nothing for a month" is the
 * failure mode that comes with it, so the outcome is written down where
 * /healthz can report it.
 */
async function recordResult(store, result) {
  if (!store) return;
  await store
    .update((s) => {
      s.auth = s.auth || {};
...
/** What /healthz says about it: whether it is armed, and how it last went. */
function status(site) {
  const last = (site && site.auth && site.auth.indexNowLast) || null;
  return {
    configured: Boolean(stored(site)),
...
module.exports = { ensureKey, keyFileFor, submit, siteUrls, generateKey, stored, recordResult, status, ENDPOINT };
