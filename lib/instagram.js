...
function reelFromFeedItem(item) {
  ...
  const permalink = pick('permalink', 'link', 'postUrl', 'url');
  const video = pick('mediaUrl', 'media_url', 'videoUrl', 'video_url', 'video');
  const poster = pick('thumbnailUrl', 'thumbnail_url', 'thumbnail', 'previewUrl', 'imageUrl', 'image_url', 'image');
  const kind = pick('mediaType', 'media_type', 'type').toUpperCase();
  ...
  return {
    ...
    poster,
    caption: captionText(pick('caption', 'text', 'title')),
    visible: true,
    source: 'instagram'
  };
}

/** Feeds arrive as a bare array, or wrapped under one of a few usual names. */
function feedItems(body) {
  if (Array.isArray(body)) return body;
  if (!body || typeof body !== 'object') return [];
  for (const key of ['data', 'media', 'posts', 'items', 'results']) {
...
function feedUrlProblem(feedUrl) {
  ...
  if (/(^|\.)instagram\.com$/i.test(host) || /(^|\.)instagr\.am$/i.test(host)) {
    return 'that is your Instagram page, not a feed. Instagram will not let a website read it. Connect the account at behold.so (free) and paste the feed URL it gives you instead';
  }
  return '';
}
