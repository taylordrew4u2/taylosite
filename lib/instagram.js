...
async function saveApp({ store, appId, appSecret }) {
  ...
  await store.update((site) => {
    ...
    site.auth.instagramApp = { appId: id, appSecret: secret || prev.appSecret || '' };
    return site;
  });
  resetCache();
}
...
function authorizeUrl(origin, env = process.env, site = null) {
  const { appId, authorize } = config(envWithApp(env, site));
  if (!appId) return '';
