...
function status(site, env = process.env) {
  ...
  return {
    ...
    // A token issued before messaging was turned on does not carry the
    // permission, however the checkbox reads now; reconnecting is what grants it.
    messagingScope: box ? Boolean(box.messaging) : Boolean(cfg.seed),
...
function box(token, expiresIn, source, now, messaging = false) {
  return {
    token,
    source,
...
async function connect({ store, code, origin, env = process.env, site = null, now = Date.now() }) {
  ...
  const next = box(long.access_token, long.expires_in, 'connected', now, messagingEnabled(merged, site));
...
async function currentToken({ store, site, env = process.env, now = Date.now() }) {
  ...
  if (store && dueSoon && oldEnough && expiresAt > now) {
    try {
      ...
      const next = box(renewed.access_token, renewed.expires_in, held.source || 'connected', now, held.messaging);
