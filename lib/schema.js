...
/**
 * A short identity fact: the location and gender that become Person.gender,
 * Person.homeLocation and the PostalAddress every crawler reads. Overlong text
 * here is already a mistake, but `str` would cut it mid-word and publish the
 * fragment as structured data — an addressRegion reading "additionally k" tells
 * every answer engine something false. Trim on a word boundary instead, so the
 * worst case is a shortened truth rather than a coined one.
 */
function fact(value, fallback, max) {
  const s = str(value, fallback, max * 8);
  if (s.length <= max) return s;
  const trimmed = s.slice(0, max).replace(/\s+\S*$/, '').replace(/[\s,;:–—-]+$/, '').trim();
...
function url(value, fallback = '') {
  ...
  // An empty box is an instruction, not a missing field: emptying a link, a
  // photo or a feed URL has to be possible, and treating "" as "leave it as it
  // was" made every URL on the site permanent once set. A key that is absent
  // still falls back — that is what lets the panel send partial updates.
  if (!s) return typeof value === 'string' ? '' : fallback;
...
function mediaPath(value, fallback = '') {
  const s = str(value, '', 600);
  // Same rule as `url`: removing a photo is a thing people do.
  if (!s) return typeof value === 'string' ? '' : fallback;
...
/** A gallery image must resolve to a real image URL or a served media path. */
function photoPath(value) {
  ...
  try {
    ...
    return ['http:', 'https:'].includes(parsed.protocol) && parsed.hostname && !parsed.username && !parsed.password ? parsed.href : '';
  } catch (_) { return ''; }
}
...
/**
 * A Wikidata item ID, however it was copied — the bare Q number, the item URL,
 * or the entity URI. Stored as the Q number, since that is the identifier and
 * every URL form can be rebuilt from it.
 */
function wikidataId(value, fallback = '') {
  const raw = str(value, '', 200);
  if (!raw) return typeof value === 'string' ? '' : fallback;
  const match = /(?:^|\/)(Q[1-9][0-9]{0,11})(?:$|[/#?])/i.exec(raw.trim());
...
function normalizeCta(input, fallback) {
  ...
  return {
    ...
    href: url(src.href, fallback.href) || fallback.href,
...
function normalizeSite(input, current) {
  ...
  const seo = {
    ...
    wikidata: wikidataId(seoSrc.wikidata, seoCur.wikidata || '')
  };
  ...
  const reels = {
    ...
    items: list(Array.isArray(reelsSrc.items) ? reelsSrc.items : reelItemsCur, 120).map((item, i) => {
      ...
      return {
        ...
        posterAlt: str(it.posterAlt, '', 500),
        caption: str(it.caption, '', 200),
        // When the clip was posted. Video results and answer engines both want
        // a date; an unparseable one is worse than none, so it is dropped.
  ...
  const photosSrc = pick('photos');
  const photosCur = cur('photos');
  const photoItemsCur = Array.isArray(photosCur.items) ? photosCur.items : [];
  const photos = {
    kicker: str(photosSrc.kicker, photosCur.kicker || 'Photos', 80),
    title: str(photosSrc.title, photosCur.title || 'Photos', 120),
    intro: longStr(photosSrc.intro, photosCur.intro || ''),
    items: list(Array.isArray(photosSrc.items) ? photosSrc.items : photoItemsCur, 100).map((item, i) => {
      const it = isObject(item) ? item : {};
      return {
        id: slugId('photo', it.id, i),
        photo: photoPath(it.photo),
        photoAlt: str(it.photoAlt, '', 500),
  ...
  const shows = list(Array.isArray(src.shows) ? src.shows : showsCur, 200).map((show, i) => {
    ...
    return {
      ...
      // The poster for the night — shown beside the listing and published as
      // the event's image, which is what a rich result actually displays.
      flyer: mediaPath(it.flyer, ''),
      flyerAlt: str(it.flyerAlt, '', 500),
      soldOut: bool(it.soldOut, false),
      visible: bool(it.visible, true)
    };
  });

  const contactSrc = pick('contact');
  const contactCur = cur('contact');
  const contact = {
    kicker: str(contactSrc.kicker, contactCur.kicker, 80),
    title: str(contactSrc.title, contactCur.title, 120),
    intro: longStr(contactSrc.intro, contactCur.intro || ''),
    to: str(contactSrc.to, contactCur.to, 160),
    subject: str(contactSrc.subject, contactCur.subject, 160),
    placeholder: longStr(contactSrc.placeholder, contactCur.placeholder || ''),
    sendLabel: str(contactSrc.sendLabel, contactCur.sendLabel, 40)
  };
...
module.exports = { normalizeSite, publicSite, isObject, photoPath };
