...
/**
 * Structured data, so search engines and social cards understand who this is
 * and when the next shows are. `<` is escaped to keep the block un-breakable.
 */
function jsonLd(site, origin) {
  const sameAs = (site.links.items || [])
  ...
  const person = {
    ...
    ...(site.brand.location ? { homeLocation: { '@type': 'Place', name: site.brand.location } } : {}),
    ...(site.home.photo && origin ? { image: origin + site.home.photo } : {}),
  ...
  const events = upcomingShows(site)
    ...
    .map((show) => ({
      ...
      location: {
        '@type': 'Place',
        name: show.venue,
        ...(show.city ? { address: show.city } : {})
      },
      performer: { '@type': 'Person', name: site.brand.name },
