...
const { photoPath } = require('./schema');
...
function navItems(site) {
  ...
  if (visiblePhotos(site).length && !has('/photos')) items.push({ id: 'nav-photos', label: 'Photos', href: '/photos', visible: true });
  return items;
}
...
const DESKTOP_PAGES = {
  ...
  '/photos': { word: 'photo album', path: 'C:\\TAYLOR\\PHOTOS', art: 'folder' },
  ...
  return `
      <span class="boot-logo">
        ...
        <span class="boot-wordmark">
          ...
          <span class="boot-wm-edition">Step-mom&#39;s Computer Edition</span>
        </span>
      </span>`;
}
...
function renderBoot(site, name) {
  const post = [
    ...
    'Detecting Dial-Up             ... Step-mom Is On The Phone',
    '',
  ...
  const users = [
    ...
    ['mom', 'Step-mom', 'Password protected. It&#39;s the dog&#39;s name.'],
  ...
  <div class="boot" hidden data-stage="post">
    ...
    <div class="boot-welcome" aria-hidden="true">
      ...
      <div class="boot-welcome-foot">
        ...
        <span class="boot-welcome-note">After you log on, do not download anything.<br>Step-mom will know. Step-mom always knows.</span>
      </div>
    </div>
...
/**
 * A stored identity value that has had prose written over it — a location
 * holding half a biography, a gender holding a truncated sentence — still has
 * to produce correct structured data. Visible copy is left exactly as saved, so
 * the mistake stays plain to whoever edits the page, but the machine-readable
 * graph must not assert it: a Place named "New York City — Taylor Drew is a ..."
 * or an addressRegion reading "additionally k" is not a shortened truth, it is a
 * coined fact, and an answer engine has no way to tell. Where the original value
 * is still recoverable it is used; where it is not, the property is dropped,
 * because saying nothing about where she lives beats saying something false.
 */
function identityValue(path, value) {
  const text = String(value == null ? '' : value).trim();
  const issue = identityIssue(path, text);
  return issue ? issue.suggestion : text;
}
 ...
 */
function jsonLd(site, { origin, canonical, title, description, current, reels = [] }) {
  const sameAs = identityLinks(site).map((l) => l.url);
  const location = identityValue('brand.location', site.brand.location);
  ...
  const galleryImages = current === '/photos' ? visiblePhotos(site).map((photo) => ({
    '@type': 'ImageObject',
    '@id': `${origin}/photos#photo-${encodeURIComponent(photo.id)}`,
    url: new URL(photo.photo, origin).href,
    contentUrl: new URL(photo.photo, origin).href,
    ...(photo.title ? { name: photo.title } : {}),
    ...(photo.photoAlt ? { description: photo.photoAlt } : {}),
    ...(photo.caption || photo.photoAlt ? { caption: photo.caption || photo.photoAlt } : {}),
    ...(photo.credit ? { creditText: photo.credit } : {}),
    about: { '@id': personId },
    isPartOf: { '@id': pageId }
  })) : [];
  // Every clip, described as the video it is. A reel wall is a grid of poster
  // images to a crawler; as VideoObjects the clips can be listed, played and
  // cited, which is the difference between "she has a reels page" and an answer
  // engine naming the clip.
  // The clips on this page, not every clip the site knows about: structured
  // data that describes something the visitor cannot see is a mismatch, and
  // page two of the wall holds different clips from page one.
  const videos = current === '/reels' ? reels
    .filter((reel) => reel && reel.visible !== false && (reel.video || reel.url))
    .map((reel) => {
      const clip = reel.video && new URL(reel.video, origin).href;
      const name = reel.caption || reel.posterAlt || `${site.brand.name} clip`;
      return {
        '@type': 'VideoObject',
        '@id': `${origin}/reels#reel-${encodeURIComponent(reel.id)}`,
        name,
        description: reel.caption || reel.posterAlt || `A clip of ${site.brand.name}.`,
        ...(reel.poster ? { thumbnailUrl: new URL(reel.poster, origin).href } : {}),
        ...(clip ? { contentUrl: clip } : {}),
        ...(reel.url ? { embedUrl: reel.url, url: reel.url } : { url: `${origin}/reels` }),
        ...(reel.published ? { uploadDate: reel.published } : {}),
        creator: { '@id': personId },
        about: { '@id': personId },
        isPartOf: { '@id': pageId }
      };
  ...
  const pageImage = { '/about': aboutImage, '/photos': galleryImages[0] }[current] || primaryImage || aboutImage;
  ...
  const person = {
    ...
    ...(site.brand.email
      ...
      : {}),
    ...(location
      ? { homeLocation: { '@type': 'Place', name: location }, address: postalAddress(location) }
      : {}),
    ...(uniqueImages.length
  ...
  const gender = identityValue('brand.gender', site.brand.gender);
  ...
  if (site.brand.accentLabel) {
    person.hasOccupation = {
      ...
      ...(location
        ? { occupationLocation: { '@type': 'City', name: location } }
        : {})
    };
  }
  ...
  const pageTypes = { '/': 'WebPage', '/about': 'ProfilePage', '/links': 'CollectionPage', '/reels': ['CollectionPage', 'VideoGallery'], '/photos': ['CollectionPage', 'ImageGallery'] };
  const webPage = {
  ...
  if (current === '/photos' && galleryImages.length) {
    webPage.mainEntity = galleryImages.map((image) => ({ '@id': image['@id'] }));
    webPage.image = galleryImages.map((image) => ({ '@id': image['@id'] }));
  }
  if (current === '/reels' && videos.length) {
    webPage.mainEntity = videos.map((video) => ({ '@id': video['@id'] }));
    webPage.video = videos.map((video) => ({ '@id': video['@id'] }));
  }
...
function layout(site, { title, description, current, body, bodyClass = '', origin = '', noindex = false, reels = [] }) {
  ...
  const favicon = [
    ...
    .filter(Boolean)
    .join('\n');
  const galleryCover = current === '/photos' ? visiblePhotos(site)[0] : null;
  const shareImage = galleryCover?.photo || site.seo.ogImage || site.home.photo;
  // The share image is often a logo rather than the hero photo, in which case
  // the hero's alt text describes the wrong picture.
  const shareImageAlt =
    (galleryCover ? galleryCover.photoAlt || galleryCover.title : site.seo.ogImage ? site.seo.ogImageAlt : site.home.photoAlt) ||
    site.home.photoAlt ||
    site.brand.name;
...
<link rel="stylesheet" href="/assets/css/retro.css?v=favorites-1">
</head>
...
<div class="win-doc">
<!-- This is the element that scrolls: the desktop window holds the page and
     the document itself never moves. A scrollable region has to be reachable
     from the keyboard, and at tabindex="-1" this one was not in the tab order,
     so End and Page Down did nothing at all until something happened to focus
     it. tabindex="0" puts it in the tab order; the skip link still works. -->
...
${origin ? jsonLd(site, { origin, canonical, title, description, current, reels }) : ''}
...
function renderShowRow(show, { compact = false } = {}) {
  ...
  const flyer =
    !compact && show.flyer
      ? `<a class="show-flyer" href="${esc(show.flyer)}" target="_blank" rel="noopener"><img src="${esc(show.flyer)}" alt="${esc(show.flyerAlt || `Flyer for ${show.venue || 'this show'}`)}" loading="lazy" decoding="async"></a>`
      : '';
...
function renderLinks(site, ctx = {}) {
  ...
  const shortcut = (link) => {
    if (/^mailto:/i.test(link.url)) return { icon: 'envelope', destination: link.url.slice(7).split('?')[0], type: 'Email' };
    let destination = 'On this site';
    try { destination = new URL(link.url).hostname.replace(/^www\./i, ''); } catch (_) {}
    const subject = `${link.label} ${destination}`;
    const art = /film|imdb|youtube|vimeo|reel/i.test(subject) ? 'film' : /podcast|press|news/i.test(subject) ? 'page' : /comedy|show|ticket/i.test(subject) ? 'folder' : 'monitor';
    return { icon: art, destination, type: /^https?:/i.test(link.url) ? 'Internet shortcut' : 'Site shortcut' };
  };
  ...
  <section class="favorites" aria-label="Favorites">
    <header class="favorites-head">
      <span class="favorites-folder">${iconArt('folder')}</span>
      <div>
        ${site.links.kicker ? `<p class="favorites-kicker">${esc(site.links.kicker)}</p>` : ''}
        <h1>${esc(site.links.title)}</h1>
        ${site.links.intro ? `<p class="favorites-intro">${esc(site.links.intro)}</p>` : ''}
      </div>
    </header>
    <div class="favorites-address"><span>Address</span><div>${iconArt('folder')}<span>Favorites<span class="favorites-separator" aria-hidden="true">›</span>${esc(site.brand.name)}</span></div></div>
    <div class="favorites-columns" aria-hidden="true"><span>Name</span><span>Location</span><span>Type</span></div>
    ...
    ${
      links.length
        ? links
            .map(
              (link) => {
                const item = shortcut(link);
                return `<li><a class="link-card favorites-link${link.featured ? ' is-featured' : ''}" href="${esc(href(link))}"${/^https?:/i.test(link.url) ? ` data-tracked-link="/go/${encodeURIComponent(link.id)}"` : ''}${external(link)}>
                  <span class="favorites-name"><span class="favorites-icon">${iconArt(item.icon, { shortcut: true })}</span><span class="favorites-copy"><span class="link-label">${esc(link.label)}</span>${link.sublabel ? `<span class="link-sub">${esc(link.sublabel)}</span>` : ''}${link.featured ? '<span class="favorites-featured" aria-label="Featured">★</span>' : ''}</span></span>
                  <span class="favorites-location">${esc(item.destination)}</span><span class="favorites-type">${esc(item.type)}</span>
                </a></li>`;
              }
            )
            .join('')
        : '<li class="favorites-empty">No shortcuts saved yet.</li>'
    }
    </ul>
    <div class="favorites-status">${links.length} ${links.length === 1 ? 'shortcut' : 'shortcuts'}<span>My Favorites</span></div>
  </section>
  ${
    upcoming.length
...
/** The pinned reels and the account's feed as one list, the pinned ones first. */
function wallReels(site, remote, { pinned = true } = {}) {
  ...
  const mine = visibleReels(site);
  const key = (r) => String(r.url || '').replace(/\/+$/, '');
  const seen = new Set(mine.map(key));
  const feed = (remote || []).filter((r) => !seen.has(key(r)));
  return pinned ? mine.concat(feed) : feed;
}

/**
 * One tile of the wall. Where a reel has its own video it plays silently on a
 * loop. Instagram's embed cannot be made to autoplay from another site — that
 * is their player's decision, not a setting — so a reel with only a permalink
 * falls back to the embed, and one with a poster falls back to the still.
 * Muted + playsinline is what lets a phone play it inline at all rather than
 * going fullscreen.
 */
function reelTile(site, reel) {
  ...
  const inner = reel.video
    ...
    : reel.poster
      ? `<img class="reel-media" src="${esc(reel.poster)}" alt="${esc(reel.posterAlt || label)}" loading="lazy" decoding="async">`
  ...
  // The id lets the browser skip a tile it already has when a page is appended.
  const id = ` data-reel="${esc(reel.id || reel.url || '')}"`;

  // An embed is already interactive; wrapping it in a link would swallow it.
  return reel.url && !(!reel.video && !reel.poster)
    ? `<a class="reel" href="${esc(reel.url)}" target="_blank" rel="noopener"${id}><span class="sr-only">${esc(label)}</span>${inner}</a>`
    : `<div class="reel"${id}>${inner}</div>`;
}

/** The tiles for one page of the wall, as a fragment the browser can append. */
function renderReelTiles(site, reels) {
  return (reels || []).map((reel) => reelTile(site, reel)).join('');
}

/**
 * The link to the next page of the wall. It is a real link so that a crawler
 * or a browser without JavaScript can still walk every reel; the script on
 * the page turns it into a sentinel that loads the next page as it scrolls
 * into view, and keeps doing so until the account runs out.
 */
function reelMore(next) {
  return next
    ? `<section class="reel-more" data-next="${esc(next)}">
    <a class="btn reel-more-link" href="/reels?after=${encodeURIComponent(next)}" rel="nofollow">More reels</a>
  </section>`
    : '';
}
 ...
 * 9:16 a reel is shot in — and as long as the account is, a page at a time.
 */
function renderReels(site, ctx = {}) {
  const head = site.reels || {};
  // The server hands over the page it has already cut; anything older gets the
  // whole wall built here so a bare render still shows every pinned reel.
  const reels = ctx.reels || wallReels(site, ctx.remote);
  ...
  ${
    reels.length
      ? `<section class="reel-grid">${renderReelTiles(site, reels)}</section>${reelMore(ctx.next)}`
      : ctx.after
        ? // Past the last page there is nothing to show but the way back.
          `<section class="reel-empty"><p class="muted">That is every reel. <a href="/reels">Back to the start</a>.</p></section>`
      : ctx.profile
...
/** Published photographs have permanent, crawlable image URLs. */
function visiblePhotos(site) {
  return ((site.photos || {}).items || [])
    .filter((photo) => photo && photo.visible !== false && photoPath(photo.photo))
    .map((photo) => ({ ...photo, photo: photoPath(photo.photo) }));
}

function renderPhotos(site, ctx = {}) {
  const head = site.photos || {};
  const photos = visiblePhotos(site);
  const body = `
  <section class="page-head photo-head">
    ${head.kicker ? `<p class="eyebrow">${esc(head.kicker)}</p>` : ''}
    <h1 class="display">${esc(head.title || 'Photos')}</h1>
    <div class="rule rule-accent"></div>
    ${head.intro ? `<p class="subhead">${esc(head.intro)}</p>` : ''}
  </section>
  ${photos.length ? `<section class="photo-gallery" aria-label="Photo gallery">
    ${photos.map((photo, index) => `<figure class="photo-card" id="photo-${esc(photo.id)}">
      <a class="photo-image-link" href="${esc(photo.photo)}" target="_blank" rel="noopener" aria-label="${esc(`Open full-size ${photo.title || 'photo'}`)}">
        <img class="photo-image" src="${esc(photo.photo)}" alt="${esc(photo.photoAlt || photo.title || `Photo of ${site.brand.name}`)}" loading="${index === 0 ? 'eager' : 'lazy'}" decoding="async">
      </a>
      <figcaption class="photo-caption">
        ${photo.title ? `<h2>${esc(photo.title)}</h2>` : ''}
        ${photo.caption ? `<p>${esc(photo.caption)}</p>` : ''}
        ${photo.credit ? `<p class="photo-credit">Photo credit: ${esc(photo.credit)}</p>` : ''}
        <a class="photo-full-size" href="${esc(photo.photo)}" target="_blank" rel="noopener">View full-size photo ↗</a>
      </figcaption>
    </figure>`).join('')}
  </section>` : '<section class="page-head"><p class="muted">Photos will be added here soon.</p></section>'}`;
  return layout(site, {
    title: `${head.title || 'Photos'} — ${site.brand.name}`,
    description: head.intro || `Photos from ${site.brand.name}’s official website.`,
    current: '/photos', bodyClass: 'page-photos', origin: ctx.origin, body,
    noindex: !photos.length
  });
}
...
function llmsTxt(site, origin = '') {
  ...
  // llms.txt exists to be read by answer engines, so it gets the same treatment
  // as the structured data: a location that has had a biography written over it
  // is published as the place it names, or not at all.
  const summary = [site.brand.accentLabel, identityValue('brand.location', site.brand.location)].filter(Boolean).join(' · ');
  ...
  const photos = visiblePhotos(site);
  if (photos.length) out.push(`- [${site.photos.title || 'Photos'}](${abs('/photos')}): photo gallery with descriptions and photographer credits where supplied`);
  out.push('');

  if (photos.length) {
    out.push('## Photos', '');
    for (const photo of photos) {
      const location = /^https?:\/\//i.test(photo.photo) ? photo.photo : abs(photo.photo);
      out.push(`- [${photo.title || photo.photoAlt || 'Photo'}](${location})${photo.caption || photo.photoAlt ? `: ${photo.caption || photo.photoAlt}` : ''}${photo.credit ? ` (Photo credit: ${photo.credit})` : ''}`);
    }
    out.push('');
  }
  ...
  out.push(`Full text: ${abs('/llms-full.txt')}`, '');
  return out.join('\n');
}

// The same site with nothing left out: past shows, every clip, the press
// quotes, the page copy. llms.txt is the index an answer engine reads first;
// this is the document it reads when it wants the whole answer, and it exists
// so the answer comes from here rather than from a guess.
function llmsFullTxt(site, origin = '') {
  const out = [llmsTxt(site, origin).trimEnd(), ''];
  const section = (heading, lines) => {
    if (!lines.length) return;
    out.push(`## ${heading}`, '', ...lines, '');
  };

  const quotes = ((site.about && site.about.quotes) || [])
    .filter((q) => String(q.text || '').trim())
    .map((q) => `- "${q.text}"${q.source ? ` — ${q.source}` : ''}`);
  section('Press', quotes);

  const past = (site.shows || [])
    .filter((show) => show.visible !== false && show.venue && !upcomingShows(site).some((up) => up.id === show.id))
    .map((show) => `- ${[show.date, [show.venue, show.city].filter(Boolean).join(', ')].filter(Boolean).join(' — ')}`);
  section('Past shows', past);

  const reels = visibleReels(site)
    .filter((reel) => reel.caption || reel.url)
    .map((reel) => {
      const when = reel.published ? ` (${reel.published.slice(0, 10)})` : '';
      const name = reel.caption || reel.posterAlt || 'Clip';
      return reel.url ? `- [${name}](${reel.url})${when}` : `- ${name}${when}`;
    });
  section('Clips', reels);

  const pages = [
    ['Home', site.home && [site.home.kicker, site.home.headline, site.home.subhead]],
    ['Links', site.links && [site.links.title, site.links.intro]],
    ['Clips', site.reels && [site.reels.title, site.reels.intro]],
    ['Photos', site.photos && visiblePhotos(site).length ? [site.photos.title, site.photos.intro] : []],
    ['Contact', site.contact && [site.contact.title, site.contact.intro]]
  ]
    .flatMap(([name, parts]) => {
      const text = (parts || []).filter((part) => String(part || '').trim());
      return text.length ? [`### ${name}`, '', ...text.map(String), ''] : [];
    });
  if (pages.length) out.push('## Page copy', '', ...pages);
...
module.exports = { renderHome, renderAbout, renderShows, renderLinks, renderReels, renderReelTiles, wallReels, renderPhotos, visiblePhotos, visibleReels, renderPlay, renderContact, renderNotFound, llmsTxt, llmsFullTxt, faviconSvg, webManifest, esc };
