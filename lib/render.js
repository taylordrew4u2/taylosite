...
const DESKTOP_PAGES = {
  '/': { word: 'home', path: 'C:\\TAYLOR\\HOME.HTM', glyph: '\u25a0', tone: 'blue' },
  '/about': { word: 'notepad', path: 'C:\\TAYLOR\\ABOUT.TXT', glyph: '\u25a5', tone: 'teal' },
  '/reels': { word: 'media player', path: 'C:\\TAYLOR\\CLIPS', glyph: '\u25b6', tone: 'red' },
  '/links': { word: 'my favorites', path: 'C:\\TAYLOR\\LINKS', glyph: '\u25a4', tone: 'slate' },
  '/play': { word: 'PLAY.EXE', path: 'C:\\TAYLOR\\PLAY.EXE', glyph: '\u2605', tone: 'violet' },
  '/contact': { word: 'new message', path: 'C:\\TAYLOR\\MAIL', glyph: '\u2709', tone: 'gold' }
};
...
function desktopPage(href) {
  ...
  const slug = String(href || '')
    ...
    .toUpperCase()
    .slice(0, 8);
  return { word: slug.toLowerCase() || 'window', path: `C:\\TAYLOR\\${slug || 'WINDOW'}`, glyph: '\u25a4', tone: 'blue' };
}
...
function renderIcons(site, current) {
  const shortcuts = navItems(site)
    ...
    .map((item) => {
      ...
      return `<li><a class="icon" href="${esc(item.href)}"${item.href === current ? ' aria-current="page"' : ''}>
        <span class="icon-tile icon-${esc(page.tone)}" aria-hidden="true">${page.glyph}</span>
  ...
  const jokes = JOKE_ICONS.map(
    (j) => `<li><button class="icon" type="button" data-dialog="${esc(j.id)}">
      <span class="icon-tile icon-${esc(j.tone)}" aria-hidden="true">${j.glyph}</span>
...
function renderShell(site, current) {
  ...
  <div class="taskbar">
    ...
    <div class="tasks"><button class="task is-active" type="button" data-window="restore">${esc(windowTitle(site, current))}</button></div>
